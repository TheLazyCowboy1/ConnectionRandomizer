...
{
    public static List<Connectible> RandomlyConnectConnectibles(List<Connectible> connectibles, float randomness = 0.5f, int triesRemaining = 3, int fruitlessTestLimit = 6)
    {
        try
        {
            ...
            while (notConnected.Count > 0)
            {
                ...
                {
                    ConnectionRandomizer.LogSomething("Could not find any notConnected to connect to connected!! notConnected.Count: " + notConnected.Count + ", free connections: " + freeConnectionCount);
                    break;
                }
                else if (lowestConnectedConn == "" || lowestNotConnectedConn == "")
                {
                    ConnectionRandomizer.LogSomething("Somehow didn't get correct connection data??? notConnected.Count: " + notConnected.Count + ", free connections: " + freeConnectionCount);
                }
                ...
                } catch (Exception ex)
                {
                    ...
                    ConnectionRandomizer.LogSomething("Connection error?? " + best.name + ": " + lowestNotConnectedConn);
                }
            ...
            ConnectionRandomizer.LogSomething(connectionPattern);
            ConnectionRandomizer.LogSomething("Time 1: " + (System.DateTime.Now.Ticks - startTime));
            ConnectionRandomizer.LogSomething("Optimization Amount: " + ((float)testsSaved / (float)totalTests));
            ...
            while (inaccessibleLeft)
            {
                ...
                foreach (Connectible c in connected)
                {
                    ...
                    foreach (string conn in c.connections.Keys)
                    {
                        ...
                        {
                            ConnectionRandomizer.LogSomething("No potential connections for " + c.name + ": " + conn);
                            c.AddPotentialConnectionsLatter(connected);
                            if (c.potentialConnections[conn].Count < 1)
                                ConnectionRandomizer.LogSomething("STILL no potential connections for " + c.name + ": " + conn);
                        }
                ...
                {
                    ConnectionRandomizer.LogSomething("Couldn't find any other free ACCESSIBLE connection!! " + lowestName + ", " + lowestConn);
                    break;
                }
            ...
            while (thingsStillUnconnected)
            {
                    ...
                    foreach (var conn in connected[i].connections)
                    {
                        if (conn.Value == "")
                        {
                            ...
                            else
                            {
                                ...
                                ConnectionRandomizer.LogSomething("No potential connections for " + connected[i].name + ": " + conn.Key);
                                connected[i].AddPotentialConnectionsLatter(connected);
                                if (connected[i].potentialConnectionScores[conn.Key].Count > 0)
                                    free.Add(conn.Key);
                                else
                                    ConnectionRandomizer.LogSomething("STILL no potential connections for " + connected[i].name + ": " + conn.Key);
                            }
                        }
                    }
                ...
                if (lowestIdx < 0)
                {
                    ConnectionRandomizer.LogSomething("Couldn't find any other free connection!! " + connected[connIdx].name + ", " + connName);
                    //continue;
                    break;
                }
            ...
            ConnectionRandomizer.LogSomething(connectionPattern);
            ConnectionRandomizer.LogSomething("Time 2: " + (System.DateTime.Now.Ticks - startTime));
            ...
            if (triesRemaining > 0)
            {
                ...
                foreach (Connectible c in connected)
                {
                    if (c.connections.Values.Contains("") || c.connections.Values.Contains(c.name))
                    {
                        ...
                        ConnectionRandomizer.LogSomething("Missing two connections!! Rerandomizing connectibles.");
            ...
            //ConnectionRandomizer.LogSomething("Calls to ConnectionScore: " + Connectible.callsToConnectionScore);
            //ConnectionRandomizer.LogSomething("Calls to ScoreDiff: " + Connectible.callsToScoreDiff);
    ...
    //{
        //ConnectionRandomizer.LogSomething(obj);
    //}
    ...
    public class Connectible
    {
        ...
        */ //changed to be configs...
        ...
        public static float CONNECTION_DISTANCE_MODIFIER = 4f;
        public static float QUAD_CONNECTION_DISTANCE_MODIFIER = 384f; //the square of CONNECTION_DISTANCE score
        public static float PLACEMENT_ANGLE_MODIFIER = 1f;
        public static float ANGLE_SCORE_MODIFIER = 3f; //set to lower because it's not important in vanilla (e.g: LF and SB)
        public static float BONUS_ANGLE_PLACEMENT_MODIFIER = 5f; //applies only when snapping a connectible into place
        ...
        public static void SetConstants()
        {
            ...
            ORIGINAL_DISTANCE_SCORE_MODIFIER = 5f * ConnectionRandomizer.Options.logicOrigDistModifier.Value; //this ought to be a config. Groups rooms together nicely
            CONNECTION_DISTANCE_MODIFIER = 4f * ConnectionRandomizer.Options.logicConnDistModifier.Value;
            QUAD_CONNECTION_DISTANCE_MODIFIER = 384f * ConnectionRandomizer.Options.logicQuadConnDistModifier.Value; //the square of CONNECTION_DISTANCE score
            PLACEMENT_ANGLE_MODIFIER = 1f * ConnectionRandomizer.Options.logicPlacementAngleModifier.Value;
            ANGLE_SCORE_MODIFIER = 3f * ConnectionRandomizer.Options.logicPureAngleModifier.Value; //set to lower because it's not important in vanilla (e.g: LF and SB)
            BONUS_ANGLE_PLACEMENT_MODIFIER = 5f * ConnectionRandomizer.Options.logicBonusAngleModifier.Value; //applies only when snapping a connectible into place
            SAME_CONN_COUNT_PENALTY = 320f * ConnectionRandomizer.Options.logicSameConnPenaltyModifier.Value; //discourages chains of two-connection-connectibles
            SCORE_POTENTIAL_MODIFIER = ConnectionRandomizer.Options.logicScorePotentialModifier.Value;
            NEXT_POTENTIAL_SCORE_MODIFIER = ConnectionRandomizer.Options.logicNextPotentialModifier.Value;
            POTENTIAL_CONNECTIONS_CAP = ConnectionRandomizer.Options.logicPotentialCapModifier.Value; //caps the number of potential connections to improve randomization times
            POTENTIAL_CONN_DIST_LIMIT = ConnectionRandomizer.Options.logicPotentialConnDistLimit.Value;
        }
    ...
    private static Vector2 RecursivelySearchForMapPos(string[] mapLines, string[] worldLines, string room, int count = 1)
    {
        ...
        foreach (string c in conns)
        {
            ...
            {
                if (count <= 1)
                    ConnectionRandomizer.LogSomething("Found gate through connections: " + room + ": " + pos.ToString());
                return pos;
            }
        }
        if (conns.Count < 1)
            ConnectionRandomizer.LogSomething("No unsearched room connections for " + room);
